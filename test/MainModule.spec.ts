...
contract('MainModule', (accounts: string[]) => {
  ...
  describe('EIP-4337', () => {
    ([{
      ...
      describe(c.name, () => {
        it('Should execute call as from self', async () => {
          ...
          const prevEntrypointBalance = await hethers.provider.getBalance(context.entryPointMock.address)

          await context.entryPointMock.mockRelay({
            sender: wallet.address,
            nonce: 1,
            initCode: [],
            callData: wallet.mainModule.interface.encodeFunctionData('selfExecute', [[{
              target: callReceiver.address,
          ...
          const diffEntrypointBalance = await hethers.provider.getBalance(context.entryPointMock.address)
            .then((b) => b.sub(prevEntrypointBalance))

          expect(diffEntrypointBalance).to.equal(95)
          expect(await wallet.mainModule.nonce()).to.equal(2)
          expect(await callReceiver.lastValA()).to.equal(111)
        })

        it('Should reject invalid signature', async () => {
          const hash = ethers.utils.hexlify(ethers.utils.randomBytes(32))
          const signature = await wallet.signDigest(ethers.utils.randomBytes(32))

          await wallet.deploy()

          const tx = context.entryPointMock.mockRelay({
            sender: wallet.address,
            nonce: 0,
            initCode: [],
            callData: wallet.mainModule.interface.encodeFunctionData('selfExecute', [[{
          ...
          await expectToBeRejected(tx, 'EIP4337InvalidSignature')
        })
        ...
        it('Should reject invalid nonce', async () => {
          ...
          await expectToBeRejected(tx, 'BadNonce')
        })

        it('Should reject bad caller', async () => {
          const hash = ethers.utils.hexlify(ethers.utils.randomBytes(32))
          const signature = await wallet.signDigest(hash)
          ...
          const tx1 = wallet.mainModule.validateUserOp(op, hash, 95)
          const tx2 = wallet.mainModule.selfExecute([{
            target: callReceiver.address,
            data: callReceiver.interface.encodeFunctionData('testCall', [111, []]),
            value: 0,
            delegateCall: false,
          ...
          await expectToBeRejected(tx1, 'EIP4337NotEntrypoint')
          await expectToBeRejected(tx2, 'OnlySelfAuth')
        })
      })
    })
  })
